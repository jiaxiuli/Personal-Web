 ...
 * @Last Modified time: 2022-07-03 16:38:29
 */
...
*/
const shape = {
    one: [[row-2,1],[row-1,0],[row-1,1],[row,1]],
    oneR1: [[row-1,0],[row-1,1],[row-1,2],[row,1]],
    oneR2: [[row-2,1],[row-1,1],[row-1,2],[row,1]],
    oneR3: [[row-2,1],[row-1,1],[row-1,2],[row-1,0]],

    two: [[row-2,1],[row-1,1],[row,1],[row+1, 1]],
    twoR1: [[row-1,0],[row-1,1],[row-1,3],[row-1,2]],

    three: [[row-2,1],[row-2,0],[row-1,1],[row-1,0]],

    four: [[row-2,1],[row-1,0],[row-1,1],[row,0]],
    fourR1: [[row-1,1],[row-1,0],[row,2],[row,1]],

    five:[[row-2,0],[row-1,0],[row-1,1],[row,1]],
    fiveR1:[[row-2,1],[row-1,1],[row-2,2],[row-1,0]],

    six:[[row-2,0],[row-2,1],[row-2,2],[row-1,2]],
    sixR1:[[row-2,0],[row-1,0],[row-2,1],[row,0]],
    sixR2:[[row-2,0],[row-1,0],[row-1,2],[row-1,1]],
    sixR3:[[row-2,1],[row-1,1],[row,1],[row,0]]
...
init();
function init() {
    for( var i = 0 ; i < can.clientWidth ; i = i + 20 ){
        for( var j = 0 ; j < can.clientHeight ; j = j + 20){
...
//初始化面板
var panel = new Array();
for( var i = 0 ; i < can.clientWidth/20 ; i ++ ){
   panel[i] = new Array();
}
for( var i = 0 ; i < can.clientWidth/20 ; i ++ ){
    for( var j = 0 ; j < can.clientHeight/20 ; j ++ ){
        panel[i][j] = 0;
    }
}
console.log(panel)
    ...
    switch (shapeStatus[0]){
        case 1:
            if(shapeStatus[1] == 0 ){
                ...
                //控制变换时可能会超出左右界限
                if(arr[0][0] >= 1 && arr[3][0] <= 13 && panel[arr[1][0]-1][arr[1][1]] != 1 ){
                    arr.forEach(function(a, b){
                ...
                shapeStatus[1] = 3;
                 ...
                 //控制变换时可能会超出左右界限
                 if(arr[0][0] > 0 && arr[3][0] < 13 && panel[arr[3][0]+1][arr[3][1]] != 1 ){
                    arr.forEach(function(a, b){
        ...
        case 2:
            if(shapeStatus[1] == 0 ){
                if(arr[2][1] < 12 && panel[arr[2][0]][arr[2][1]+2] != 1 ){
                    arr.forEach(function(a, b){
                ...
                if(arr[0][0] > 0 && arr[3][0] < 12 && panel[arr[1][0]-1][arr[1][1]] != 1 
        ...
        case 4:
            if(shapeStatus[1] == 0 ){
                if(arr[2][1] < 13 && panel[arr[2][0]][arr[2][1]+1] != 1){
                    arr.forEach(function(a, b){
            ...
            case 5:
                if(shapeStatus[1] == 0 ){
                    ...
                    if(arr[0][0] > 0 && arr[3][0] < 13 &&  panel[arr[1][0]+1][arr[1][1]] != 1 ){
                        arr.forEach(function(a, b){
                ...
                case 6:
                    if(shapeStatus[1] == 0 ){
                        ...
                        //控制变换时可能会超出左右界限
                        if(arr[0][0] >= 1 && arr[3][0] <= 13 && arr[2][1] < 13 && panel[arr[1][0]][arr[1][1]+2] != 1){
                            arr.forEach(function(a, b){
                        ...
                        if(arr[2][0] < 13 && panel[arr[2][0]+1][arr[2][1]] != 1){
                            arr.forEach(function(a, b){
                         ...
                         //控制变换时可能会超出左右界限
                         if(arr[0][0] > 0 && arr[3][0] < 13 && panel[arr[3][0]+1][arr[3][1]] != 1 && arr[1][1] < 13){
                            arr.forEach(function(a, b){
...
function checkIfFull(){
    var FullLine = [];
    for(var i = 0 ; i < panel[0].length ; i++ ){
        var b = true;
